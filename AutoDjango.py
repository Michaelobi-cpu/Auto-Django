...
def html2django(zipfile, static_folder, templates_folder):
    ...
    #init to enable the color feature in the cmd
    init()
    subprocess.run("python -m pip install --upgrade pip", stdout=subprocess.DEVNULL if os_version=="windows 10" else subprocess.PIPE, shell=True if os_version in ["Linux", "Darwin"] else False)
    images = []
